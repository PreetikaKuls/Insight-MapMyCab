...
CREATE TABLE ref_tbl AS SELECT uid as cabID, year(from_unixtime(time)) AS year, month(from_unixtime(time)) AS month,
day(from_unixtime(time)) AS day, hour(from_unixtime(time)) AS hour, from_unixtime(time, 'EEE') as dow, occ, pickup, dropoff from master_tbl;

-- Hourly aggregations
CREATE TABLE hour_agg
AS SELECT * FROM
( SELECT hour, day, month, year, dow, AVG(occ) as occ, SUM(pickup) as pickups, SUM(dropoff) as dropoffs
FROM master_tbl GROUP BY year, month, dow, day, hour 
...
FROM master_tbl GROUP BY year, month, day 
